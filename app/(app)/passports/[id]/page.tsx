...
import { PASSPORTS, PASSPORT_BY_ID } from "@/lib/data/passports";
...
export default function PassportDetailClient() {
  ...
  const passport = PASSPORT_BY_ID[id];
  const [favorite, setFavorite] = useState(passport?.favorite ?? false);
  const [copied, setCopied] = useState(false);

  if (!passport) notFound();

  const project = passport.meta.projectId ? PROJECT_BY_ID[passport.meta.projectId] : null;
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {project ? (
              <Link href={`/app/workspace/${project.id}`}>
          ...
          <p className="mt-1.5 text-xs text-faint">
            Created {timeAgo(passport.createdAt)} · updated {timeAgo(passport.updatedAt)} · used {passport.usageCount}× · {passport.wordCount.toLocaleString()} words
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            onClick={() => {
              navigator.clipboard?.writeText(packText).catch(() => undefined);
              setCopied(true);
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { label: "Next run cost", value: `~${formatMoney(passport.meta.estimatedNextCost)}` },
          { label: "Best model", value: MODEL_BY_ID["gemini-flash"]?.name.split(" ").slice(0, 2).join(" ") },
          { label: "Favorites", value: `${PASSPORTS.filter((p) => p.favorite).length}` },
          { label: "Tags", value: passport.meta.tags.length.toString() },
        ].map((s) => (
